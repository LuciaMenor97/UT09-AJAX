        ...
        function VideoSystem() {
            ...
            this.escribirTodo = function () {
                ...
                for (var i = 0; i < _categories.length; i++) {
                    categorias.push(_categories[i].getObject());
                    categorias[i].producciones = [];
                    for (var j = 0; j < _categories[i].production.length; j++) {
                        categorias[i].producciones.push(_categories[i].production[j].getObject());
                    }
                }
                arrayServidor.push(categorias);

                //producciones
                for (var i = 0; i < _production.length; i++) {
                    producciones.push(_production[i].getObject());
                }
                arrayServidor.push(producciones);

                //directores
                for (var i = 0; i < _directors.length; i++) {
                    directores.push(_directors[i].getObject());
                    directores[i].producciones = [];
                    for (var j = 0; j < _directors[i].production.length; j++) {
                        directores[i].producciones.push(_directors[i].production[j].getObject());
                    }
                }
                arrayServidor.push(directores);
                
                //actores
                for (var i = 0; i < _actors.length; i++) {
                    actores.push(_actors[i].getObject());
                    actores[i].interpretaciones = [];
                    for (var j = 0; j < _actors[i].interpretaciones.length; j++) {
                        actores[i].interpretaciones.push(_actors[i].interpretaciones[j].getObject());
                    }
                }
                arrayServidor.push(actores);


                var json = JSON.stringify(arrayServidor);
                ...
                xmlhttp.open("POST", "crearRespaldo.php", true);
                xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                xmlhttp.send("respaldo=" + json);
            }
